...
它**不是**新闻聚合器。通用 AI 新闻市面上已有成熟产品（TLDR AI、Import AI），自建的边际价值低而脆弱性高。
...
**明确禁止**：任何第三方 RSS 镜像 / RSSHub 实例 / 聚合器（例如 `rsshub.*/anthropic/news`）。
...
### 6.1 静默日（预期占约 40% 的工作日）

当 `max(score) < SILENT_THRESHOLD`（初始值 0.55，两周后校准）**且 §5 厂商变动为空**：

```
主题：AI Radar — 2026-08-15 — 无需关注

今天没有需要你关注的进展。

（本周累计：3 个 Must Know，5 个技能雷达条目 → reports/2026-W33.md）
```
...
```
主题：AI Radar — 2026-08-15 — 1 条厂商变动，2 条 Must Know

━━━ 90 秒版 ━━━
⚡ [DeepSeek] 定价页变动：新增峰谷计价，非高峰半价，2026-08-16 UTC 生效
🔥 [模型] Anthropic 发布 X，定价降至 $Y/百万 token（较上代 -40%）
🔥 [工具] LangChain 0.4 移除 Z 模块，你的 A 作业需要改 import
👩‍💻 [技能] cloudflare/computer +2,856 星/周 — Agent 沙箱 — 求职相关性:高
👀 [观察] cactus-compute/needle 14MB 端侧模型，值得留意但还早

━━━ 详情 ━━━
（每条 3–5 句，含来源链接与 confidence 标记；厂商变动附 diff 行）

━━━ 本次运行 ━━━
候选 412 → 去重后 88 → 过闸门 61 → 入选 5
厂商监控：通道A 3条 | 通道B 1条 | 通道C 1处变更
LLM: deepseek-v4-flash | 用时 47s | 完整性排除 7 个（3 疑似刷星）
```

**末尾的运行统计是刻意保留的** —— 它让你一眼看出管道是否健康，也是发现静默失败的第一道防线。特别注意"通道A 0条"持续多天意味着 Anthropic 页面解析已失效（§5.2）。
...
> **脚注（非约束）**：cron 08:30 北京时间 = 00:30 UTC，恰好落在高峰窗口开始（01:00 UTC）之前，享受非高峰半价。这是巧合不是设计，金额也是分币级 —— **不要为它牺牲发送时间的合理性**。
